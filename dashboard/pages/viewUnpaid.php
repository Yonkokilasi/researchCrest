    ...
    <div id="preloader">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <small class = "breadcrumb pull-right"><a href = "index.php">Dashboard</a> / <a href = "unpaid.php">Unpaid Orders</a> / View Order</small>
                </h3>
                        <?php
        ...
        ?>
        <?php
        if(mysqli_num_rows(mysqli_query($Con, "SELECT * FROM `order` WHERE orderId = '$orderId' AND orderStatus = 'unpaid'"))>0)
        {            
        ?> 
        <div class='alert2 alert-info alert-dismissable'>
            <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
            <?php
            $row      = mysqli_fetch_assoc(mysqli_query($Con, "SELECT clientId FROM `order` WHERE orderId = '$orderId'"));
            $clientId = $row['clientId'];

            $row = mysqli_fetch_assoc(mysqli_query($Con, "SELECT * FROM `registration` WHERE reg_id = '$clientId'"));
            $regId = $row['reg_id'];
            $email = $row['email'];
            echo "Order posted by: (".$regId.") Email: ".$email;
            ?>
        </div>
            </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form method="POST" action="php/messages/sendMsg-unpaid.php">
                ...
                <div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          ...
                          <div class="modal-footer">
                              <form method="POST" action="php/redirect.php?page=viewPosted.php&id=<?=$orderId?>">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        <li><a href="#assign" data-toggle="tab">Assign Writer</a>
                        </li>
                        <li style="padding-left: 10px;">
                            <form method="" action="">                            
                            
                            <button type="submit" name="apprv" class="btn btn-success" id="validatePaid">
                            <i class="fa fa-check-square"></i>
                            Validate Order</button>
                            
                            <button type="submit" name="apprv" class="btn btn-danger" id = 'deleteUnpaid'>
...
<!-- /.col-lg-6 -->
        ...
        <hr/>
        <form method="POST" action="php/writerAssign.php">
        <table>
            <tr style="border-spacing: 5px">
                ...
                <input type="hidden" name="orderId" value="<?php echo $orderId;?>">
                    <?php
                      $sql = "SELECT reg_id, user_name FROM `registration` WHERE user_type = 'writer'";
                      $result = mysqli_query($Con, $sql);
...
Order nolonger appears under unpaid...
</div>";
}
?>               
        </div>
...
<script type="text/javascript">
  //Confirm Take
$("#deleteUnpaid").on( 'click', function () {
    reset();
    alertify.confirm("This order will be permanently DELETED, are you sure you want to delete ?", function (e) {
        if (e) {
            // alertify.success("You've clicked OK");
            window.location.href ='php/deleteUnpaid.php?id=<?=$orderId?>';  
        } else {
...
//Validate paid order
$("#validatePaid").on( 'click', function () {
    reset();
    alertify.confirm("Are you sure you want to validate this order ?", function (e) {
        if (e) {
            // alertify.success("You've clicked OK");
            window.location.href ='php/validateOrder.php?id=<?=$orderId?>';  
        } else {
            alertify.error("You've clicked Cancel");
        }
    });

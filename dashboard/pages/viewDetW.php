...
$statement = "`order` WHERE orderStatus = '$orderStatus' AND writerId = '$writerId'";
$cr = "SELECT * FROM `registration` WHERE user_type = 'client'";
$tl = mysqli_query($Con, $cr);
$np = mysqli_num_rows($tl);

$cr = "SELECT * FROM `order` WHERE orderStatus = '$orderStatus' AND writerId = '$writerId'";
$tl = mysqli_query($Con, $cr);
$np = mysqli_num_rows($tl);
?>
<!DOCTYPE html>
...
<title>Orders Details | Research Paper Web</title>
...
<div id="preloader">
    ...
    <div class="row" >
        <div class="col-lg-12">
            ...
            Orders Details
            (<?php echo $np;?>)
            <small class = "breadcrumb pull-right"><a href = "index.php">Dashboard</a> / <a href = "students.php">Students</a> / Orders Details / </small>
            </h3>
        </div>
        ...
        WHERE orderStatus = '$orderStatus' AND writerId = '$writerId'  LIMIT {$startpoint} , {$limit};";
        ...
        <td style="color: #F6059E; font-weight: bolder;" >
            <?php
        ...
        if ($check<1) {
            echo "There are no orders to view at the monent";
        }
        ?>
        </td>
        </tr>

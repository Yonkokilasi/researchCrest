...
$sql1 = ("UPDATE `order` SET `order`.orderStatus = 'done' WHERE `order`.clientId = '$clientId' AND `order`.orderId = '$id'");
$query1=mysqli_query($Con, $sql1) or mysql_error();

$sql2 = ("UPDATE `payment` 
	SET `payment`.clientId 		= '$clientId', 
		`payment`.eligible 		  = 'eligible',
    `payment`.dateDue       = '$time'		
...
$sql3 = ("INSERT INTO `r_p_w`.`notifications` (`id`, `writerId`, `studentId`, `userType`, `notification`, `status`) 
    VALUES (NULL, '$writerId', '', 'writer', 'You order # $id, has been approved !', 'not viewed')");
...
$account_email2_x  = $row_x['email'];
$user_name2_x      = $row_x['user_name'];
    ...
    <strong>Item:</strong> Order #'.$id.' <br/>
    <strong>Incident:</strong> Order has been approved by Admin<br/>
    <p>&nbsp;</p>
    Cheers,<br/>
...
$sql_x1            = "SELECT email, user_name FROM registration WHERE reg_id = '$clientId'";
$query_x1          = mysqli_query($Con, $sql_x1);
$row_x1            = mysqli_fetch_assoc($query_x1);
$account_email2_x1  = $row_x1['email'];
$user_name2_x1      = $row_x1['user_name'];

    $to       = $account_email2_x1;
    $name     = $user_name2_x1;
    $subject  = 'New notification on '.$date2.' GMT';
    $messages = '<html>
    <head>
 ...
 if ($query and $query1 and $query2 and $query3 and $send_mail and $send_mail2) {

  mysqli_query($Con,"COMMIT");

  $_SESSION['msg']="Order has been approved";
